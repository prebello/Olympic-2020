...
def plotROC(prediction,y_test_, pwd):
    # choose score cuts:
    cuts = np.linspace(0,1,500);
    n_truePos = np.zeros(len(cuts));
    n_falsePos = np.zeros(len(cuts));
    n_TotPos = len(np.where(y_test_==1)[0])
    for i,cut in enumerate(cuts):
       y_pred = np.array([i>cut for i in predictions])
       n_truePos[i] = len(np.where(predictions[y_test_==1] > cut)[0]);
       n_falsePos[i] = len(np.where(predictions[y_test_==0] > cut)[0]);
       if i%50 ==0:
         ascore = accuracy_score(y_test_,y_pred)
         pscore = precision_score(y_test_,y_pred)
         rscore = recall_score(y_test_,y_pred)
...
Dataset_signal = pd.concat([DataSet_signal_exclusivo,DataSet_signal_semiexclusivo], axis=0)
Dataset_signal_ = pd.concat([Dataset_signal,weight_signal],axis=1)
Dataset_signal_['target'] = 1
Dataset_backgraund_ = pd.concat([DataSet_backgraund,weight_backgr],axis = 1)
Dataset2 = pd.DataFrame(pd.concat([Dataset_signal_,Dataset_backgraund_], axis = 0))
dataset_ = Dataset2.rename(columns={0:'Pt', 1:'MassaInvariante', 2:'Acoplanaridade', 3: 'DeltaEta', 4:'VerticePrimario', 5:'almir1', 6:'almir2', 7:'target', 8:'pesos'})

dataset_train, dataset_test = train_test_split(dataset_, train_size = 0.3)
TrainData, ValiData= train_test_split(dataset_train,test_size = 0.05)

y_train_ = TrainData['target']
y_test_ = dataset_test['target']
ValiLabel = ValiData['target']

pesos_train = TrainData['pesos']
pesos_test = dataset_test['pesos']
pesos_vali = ValiData['pesos']

TrainData = TrainData.loc[:,'Pt':'almir2']
dataset_test = dataset_test.loc[:,'Pt':'almir2']
ValiData = ValiData.loc[:,'Pt':'almir2']
...
clf_LGBM = clf_LGBM.fit(TrainData, y_train_, sample_weight = pesos_train)
prediction_LGBM = pd.DataFrame(clf_LGBM.predict(dataset_test))
predict_proba_LGBM = pd.DataFrame(clf_LGBM.predict_proba(dataset_test))

from mlxtend.plotting import plot_decision_regions,plot_learning_curves

plot_learning_curves(np.array(TrainData), np.array(y_train_), np.array(dataset_test), y_test_, clf = clf_LGBM, print_model = False)
plt.style.use(hep.style.ROOT)
plt.tight_layout()
plt.savefig('/home/matheus/nTuplas/algoritmos/graficos/graficos_lightgbm/plot_learning_curves.pdf')
plt.close()
